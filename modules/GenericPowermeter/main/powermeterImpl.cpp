...
void powermeterImpl::init() {
    ...
    if (found != std::string::npos) {
        ...
        try {
            json powermeter_registers = Everest::load_yaml(generic_powermeter::getModelConfigPath() + config.model + ".yaml");
            this->init_register_assignments(std::move(powermeter_registers));
            this->init_default_values();
        } catch (const std::exception& e) {
            EVLOG_error << "opening file \"" << config.model << ".yaml\" from path " << generic_powermeter::getModelConfigPath() << "\" failed: " << e.what();
...
int powermeterImpl::assign_register_data(const json& registers, 
                                         const PowermeterRegisters register_type,
    ...
    try {
        if(registers.contains(register_selector)) {
            if(registers.at(register_selector).at("num_registers") > 0) {
                ...
                data.start_register_function    = select_modbus_function((const uint8_t)registers.at(register_selector).at("function_code_start_reg"));
                ...
                data.exponent_register_function = select_modbus_function((const uint8_t)registers.at(register_selector).at("function_code_exp_reg"));
...
void powermeterImpl::assign_register_sublevel_data(const json& registers, 
                                                   const PowermeterRegisters& register_type,
                                                   const std::string register_selector, 
                                                   const std::string sublevel_selector,
    ...
    sublevel_data.start_register_function    = select_modbus_function((const uint8_t)registers.at(register_selector).at(sublevel_selector).at("function_code_start_reg"));
    ...
    sublevel_data.exponent_register_function = select_modbus_function((const uint8_t)registers.at(register_selector).at(sublevel_selector).at("function_code_exp_reg"));
...
void powermeterImpl::readRegister(const RegisterData& register_config) {
    ...
    if (register_config.start_register_function == READ_HOLDING_REGISTER) {
        register_response = mod->r_serial_comm_hub->call_modbus_read_holding_registers(config.powermeter_device_id, register_config.start_register, register_config.num_registers, modbus_pause_between_messages);
    }

    if (register_config.start_register_function == READ_INPUT_REGISTER) {
        register_response = mod->r_serial_comm_hub->call_modbus_read_input_registers(config.powermeter_device_id, register_config.start_register, register_config.num_registers, modbus_pause_between_messages);
    }

    if (register_config.exponent_register != 0) {
        if (register_config.exponent_register_function == READ_HOLDING_REGISTER) {
            exponent_response = mod->r_serial_comm_hub->call_modbus_read_holding_registers(config.powermeter_device_id, register_config.exponent_register, register_config.num_registers, modbus_pause_between_messages);
        }

        if (register_config.exponent_register_function == READ_INPUT_REGISTER) {
            exponent_response = mod->r_serial_comm_hub->call_modbus_read_input_registers(config.powermeter_device_id, register_config.exponent_register, register_config.num_registers, modbus_pause_between_messages);
        }
    }
...
float powermeterImpl::merge_register_values_into_element(const RegisterData& reg_data, 
                                                         const int16_t exponent,
                                                         const types::serial_comm_hub_requests::Result& reg_message) {
    uint32_t value{0};

    if (reg_message.value.has_value()) {
        if (reg_data.num_registers <= 2) {
            for (int8_t i = (reg_data.num_registers - 1); i >= 0; i--){
                value |= reg_message.value.get()[i] << (16 * i);
            }
            return float(value * reg_data.multiplier * pow(10.0, exponent));
        } else {
